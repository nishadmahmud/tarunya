...
/**
 * Fetch Book Fair best selling products (with caching)
 * @returns {Promise<Object>} Book Fair best sellers data
 */
export async function getBookFairBestSellersFromServer() {
    if (!isApiConfigured()) return { success: true, data: [] };
    return getCachedOrFetch('book-fair-best-sellers', async () => {
        const res = await fetch(
            `${process.env.NEXT_PUBLIC_API}/public/best-sellers/${process.env.NEXT_PUBLIC_USER_ID}`,
            { next: { revalidate: 300 } }
        );
        if (!res.ok) throw new Error(`Failed to fetch book fair best sellers. Status ${res.status}`);
        return res.json();
    });
}
...
export async function getBannerFromServer() {
    if (!isApiConfigured()) return { success: true, banners: [] };
...
/**
 * Fetch upcoming products/books (with caching)
 * @returns {Promise<Object>} Upcoming products data
 */
export async function getUpcomingProductsFromServer() {
    if (!isApiConfigured()) return { success: true, data: [] };
    return getCachedOrFetch('upcoming_products', async () => {
        const res = await fetch(
            `${process.env.NEXT_PUBLIC_API}/public/upcoming-products/${process.env.NEXT_PUBLIC_USER_ID}`,
            { next: { revalidate: 300 } }
        );
...
/**
 * Fetch ebook products (with caching)
 * Uses general products feed and filters likely ebook items.
 * @returns {Promise<Object>} Ebook products data
 */
export async function getEbooksFromServer() {
    if (!isApiConfigured()) return { success: true, data: [] };
    return getCachedOrFetch('ebook_products', async () => {
        const res = await fetch(
            `${process.env.NEXT_PUBLIC_API}/public/products/userwise?user_id=${process.env.NEXT_PUBLIC_USER_ID}&page=1&per_page=120&category_id=0`,
            { next: { revalidate: 300 } }
        );
        if (!res.ok) throw new Error("Failed to fetch ebook products");
        const json = await res.json();
        const items = Array.isArray(json?.data?.data)
            ? json.data.data
            : (Array.isArray(json?.data) ? json.data : []);

        const isLikelyEbook = (p) => {
            const textHaystack = [
                p?.type,
                p?.product_type,
                p?.book_type,
            ...
            return (
                Number(p?.is_ebook || p?.ebook || 0) === 1 ||
                Boolean(p?.pdf_file) ||
                textHaystack.includes("ebook") ||
                textHaystack.includes("e-book") ||
                textHaystack.includes("ইবুক") ||
                textHaystack.includes("ই-বুক")
            );
        };

        const ebooks = items.filter(isLikelyEbook);
...
export async function getCampaigns() {
    if (!isApiConfigured()) return { success: true, campaigns: { data: [] } };
    return getCachedOrFetch('campaigns', async () => {
        try {
            ...
            return { success: false, campaigns: { data: [] } };
        }
    });
}
...
/**
 * Fetch list of all public authors (with caching)
 * @returns {Promise<Object>} List of authors data
 */
export async function getAuthorsList() {
    if (!isApiConfigured()) return { success: true, data: [] };
    return getCachedOrFetch('authors_list_v2', async () => {
        try {
            const res = await fetch(
                `${process.env.NEXT_PUBLIC_API}/public/authors?user_id=${process.env.NEXT_PUBLIC_USER_ID}`,
                { next: { revalidate: 300 } }
            );
...
/**
 * Fetch products for a specific author (with caching)
 * @param {number|string} authorId - The author ID
 * @param {number} page - Page number (default: 1)
 * @param {number} perPage - Items per page (default: 20)
 * @returns {Promise<Object>} Author products response
 */
export async function getAuthorWiseProducts(authorId, page = 1, perPage = 20) {
    if (!isApiConfigured()) return { success: true, data: [] };
    const cacheKey = `author_products_${authorId}_page_${page}_per_${perPage}`;
    return getCachedOrFetch(cacheKey, async () => {
        try {
            const res = await fetch(
                `${process.env.NEXT_PUBLIC_API}/public/author-wise-products?author_id=${authorId}`,
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
            ...
            console.error(`Failed to fetch author products for ID ${authorId}:`, error);
            return { success: false, data: [] };
        }
    });
}
...
/**
 * Fetch available payment types (Cash, SSL, etc)
 * @param {string|number} userId
 */
export async function getPaymentTypes(userId) {
    if (!userId) return null;
    try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API}/payment-type-list/${userId}`);
 ...
 * @param {Object} payload The payment initiation payload
 */
export async function initiateSslPayment(payload) {
    try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API}/payment/initiate`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
        ...
        if (!res.ok) throw new Error("Failed to initiate SSL payment");
        return await res.json();
    } catch (error) {
