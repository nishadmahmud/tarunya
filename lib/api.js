...
/**
 * Fetch ebook products from the dedicated ebook endpoint (with caching)
...
export async function getEbooksFromServer() {
    ...
    return getCachedOrFetch('ebook_products', async () => {
        const res = await fetch(
            `${process.env.NEXT_PUBLIC_API}/public/ebook-products/${process.env.NEXT_PUBLIC_USER_ID}`,
            { next: { revalidate: 300 } }
        );
        if (!res.ok) throw new Error("Failed to fetch ebook products");
        const json = await res.json();
        
        // The new API is expected to return only ebooks, so we can return the data directly
