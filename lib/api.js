...
/**
 * Fetch author details by ID (with caching)
 * @param {string|number} authorId - Author ID
 * @returns {Promise<Object>} Author details data
 */
export async function getAuthorById(authorId) {
    ...
    return getCachedOrFetch(cacheKey, async () => {
        const res = await fetch(
            `${process.env.NEXT_PUBLIC_API}/public/authors/${authorId}`,
            { next: { revalidate: 300 } }
        );
        if (!res.ok) throw new Error(`Failed to fetch author details for ID ${authorId}`);
        return await res.json();
    });
}
...
export async function getAuthorsList() {
    ...
    return getCachedOrFetch('authors_list_v2', async () => {
        ...
        if (!res.ok) throw new Error("Failed to fetch developers/authors list");
        return await res.json();
    });
}
