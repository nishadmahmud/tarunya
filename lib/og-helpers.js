...
// Fetch Bengali font for ImageResponse (Satori requires TTF/OTF, NOT woff2)
let fontCache = null;
export async function getBengaliFont() {
    if (fontCache) return fontCache;

    // Do NOT send a browser User-Agent — without it, Google Fonts returns TTF
    // which is the format Satori/ImageResponse requires
    const res = await fetch(
        'https://fonts.googleapis.com/css2?family=Noto+Sans+Bengali:wght@700&display=swap'
    );
    const css = await res.text();

    // Match TTF URL first (preferred by Satori), then any URL as fallback
    const ttfMatch = css.match(/url\((https:\/\/[^)]+\.ttf)\)/);
    const anyUrlMatch = css.match(/url\((https:\/\/[^)]+)\)/);

    const fontUrl = ttfMatch?.[1] || anyUrlMatch?.[1];

    if (!fontUrl) {
        // Ultimate fallback: fetch TTF directly from Google Fonts static URL
        const fallbackRes = await fetch(
            'https://fonts.gstatic.com/s/notosansbengali/v20/rCSiy6AOMB8AxRe_LZS1S_hOyLBn0x1HpaRGMg.ttf'
        );
