...
export async function getBengaliFont() {
    ...
    const res = await fetch(
        'https://fonts.googleapis.com/css2?family=Noto+Sans+Bengali:wght@400;700;800&display=swap'
    );
    const css = await res.text();
    // Extract the first woff2 URL for weight 700+
    const fontUrlMatch = css.match(/src: url\(([^)]+)\) format\('woff2'\)/);
    if (!fontUrlMatch) {
        // Fallback: fetch the full family
        const allMatches = [...css.matchAll(/url\(([^)]+\.woff2)\)/g)];
        if (allMatches.length > 0) {
            const fontRes = await fetch(allMatches[0][1]);
            fontCache = await fontRes.arrayBuffer();
            return fontCache;
        }
        throw new Error('Could not find Bengali font URL');
    }
